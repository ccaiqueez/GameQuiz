...
{
    class C_DalQuestoesDadosTecnologia
    {
        ...
        public void CadastrarQuestoesCinema(C_ControleQuestoesDados _questoesDados)
        {
            try
            {
                ...
                cmd.CommandText = "INSERT INTO Quiz_Tecnologia(ID_PERGUNTA,PERGUNTA, A, B, C, D, RESPOSTA) VALUES (@ID_PERGUNTA,@PERGUNTA, @A, @B, @C, @D, @RESPOSTA)";
        ...
        public void EditarQuestaoTecnologia(C_ControleQuestoesDados _questaoEditar)
        {
            try
            {
                conectionDB.Conectar();
                cmd.Connection = conectionDB.ObjetoConexao;
                cmd.CommandText = "UPDATE Quiz_Tecnologia SET ID_PERGUNTA = @ID_PERGUNTA, PERGUNTA = @PERGUNTA, A = @A, B = @B, C = @C, D = @D, RESPOSTA =  @RESPOSTA WHERE ID_PERGUNTA = @ID_PERGUNTA";
        ...
        public void DeletarQuestaoCinema(C_ControleQuestoesDados _questaoEditar)
        {
            try
            {
                conectionDB.Conectar();
                cmd.Connection = conectionDB.ObjetoConexao;
                cmd.CommandText = "DELETE FROM Quiz_Tecnologia WHERE ID_PERGUNTA = @ID_PERGUNTA";
                cmd.Parameters.AddWithValue("@ID_PERGUNTA", _questaoEditar.ID_Pergunta);
                cmd.ExecuteNonQuery();
            }
            catch (SqlException error)
        ...
        public DataTable ConsultaDeDados()
        {
            ...
            SqlDataAdapter dapter = new SqlDataAdapter("SELECT * FROM Quiz_Tecnologia", conectionDB.ObjetoConexao);
